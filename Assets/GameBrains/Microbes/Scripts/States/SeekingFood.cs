...
namespace GameBrains.Microbes.Scripts.States
{
    ...
    /// </summary>
    public class SeekingFood : State<Microbe>
    {
        private static SeekingFood instance;

        /// <summary>
        /// Prevents a default instance of the SeekingFood class from being created.
        /// </summary>
        private SeekingFood()
        {
            if (null != instance)
        ...
        /// <summary>
        /// Gets the accessor for the <see cref="SeekingFood"/> singleton instance.
        /// </summary>
        public static SeekingFood Instance
        {
            get
            {
                if (null == instance)
		...
		{
            microbe.UpdateStateDisplay("Hungry");
        }
        ...
        public override void Execute(Microbe microbe)
        {
            ...
            if (nearbyMicrobes.Count > 0)
            {
                foreach (Microbe nearbyMicrobe in nearbyMicrobes)
                {
                    ...
                    MessageDispatcher.Instance.DispatchMsg(
                        ...
                        microbe.ID,
                        nearbyMicrobe.ID,
                        (MessageTypes)MicrobeMessageTypes.YouJustGotSwallowed,
            ...
            if (microbe.IsHungry)
            {
                ...
                microbe.Attractor.radius = 50 * microbe.Hunger; // as hunger increases so does radius
                microbe.Attractor.AttractTypes = microbe.FoodTypes; // could change food type

                // If the microbe gets hungry enough, allow it to eat its own kind
                if(microbe.IsCannibalistic) {
					Debug.Log("Microbe " + microbe.name + " has turned cannibalistic");
                	microbe.FoodTypes = microbe.FoodTypes | microbe.MicrobeType;
                }
            }
        }
